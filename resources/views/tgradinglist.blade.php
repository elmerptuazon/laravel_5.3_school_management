...
@section('content')
<div id="myNav" class="{{$grading_teacher_edit->status == 1 ? 'hide' : 'overlay'}}">
  <div class="{{$grading_teacher_edit->status == 1 ? 'hide' : 'overlay-content'}}">
      <h1>Please consult admin to enable editing of the grades.</h1>
  </div>
</div>
<div class="row">
        <div class="col-md-12">
          ...
          <div class="box box-success">
            ...
            <form method="post" action="{{ route('addtask') }}" role="form">
            @csrf
            <div class="box-body">
              <div class="row">
                  ...
                  <div class="col-xs-12 col-md-offset-1 col-md-3">
                    ...
                    <input id="task_student_grade" type="text" class="hide" name="task_grade" value="{{$grade}}">             
                    <input id="task_student_section" type="text" class="hide" name="task_section" value="{{$section}}">
                    <input id="task_student_subject" type="text" class="hide" name="task_subject" value="{{$subject}}">
                    <input type="text" class="hide" name="task_teacher_id" value="{{$teacheruser->id}}">                               
                  </div>
        ...
        <form method="post" id="addScoreForm" action="{{ route('addscore') }}" role="form">
        @csrf
          <div class="col-md-12">
                            ...
                            <td>
                              <div class="row">
                                <div class="col-xs-12 col-md-6 " style="padding-right: 5px;">
                                  ...
                                  <input class="hide" id="studentID" type="text" style="width: 45px;" name="task_score[{{ $loop->index }}][ID]" value="{{ $student->id }}">
                      ...
                      <div class="box box-success">
                        ...
                        <div class="box-body">
                          ...
                          <div class="box-footer">
                            <button class="btn btn-sm btn-primary center-block" id="characterSubmit" type="submit">Submit</button>
                          </div>
                        </div>
                      </div>
                    </div>
...
$('#selectTasks').on('change', function() {
    ...
    window.location.href = '/tgrading/list/'+taskSubjectValue+'/'+taskGradeValue+'-'+taskSectionValue+'/tasklist/' + taskValue;
});

var yearToday = new Date().getFullYear();

// $('.schoolYear,#qualitative_school_year',).append($('<option>', {value:(yearToday-1)+'-'+yearToday, text:yearToday-1+'-'+yearToday}));
...
$('#search_qualitative_student').on('click', function() {
  ...
  if(schoolYear == '' || period == '' || strSplit[0] == '') {
    ...
    window.location.href = '/tgrading/list/'+taskSubjectValue+'/'+taskGradeValue+'-'+taskSectionValue+'/qualitative/score/' + strSplit[0] + '/' + period + '/' + schoolYear;
